...
def store_data():
    ...
    if not (name and phone_number and address and aadhaar_number and dob and gender):
    ...
    try:
        with sqlite3.connect(DATABASE) as conn:
            cursor = conn.cursor()
            cursor.execute('''
                INSERT INTO person (name, phone_number, address, aadhaar_number, dob, gender)
                VALUES (?, ?, ?, ?, ?, ?)
            ''', (name, phone_number, address, aadhaar_number, dob, gender))
            conn.commit()
...
def user_register():
    if request.method == 'POST':
        ...
        phone = request.form['phone']  # Ensure this line is properly indented
        metamask_address = request.form['metamask_address']

        # Insert the new user into the USERS table
        conn = sqlite3.connect('user_data.db')
        cursor = conn.cursor()
        cursor.execute("INSERT INTO USERS (USERNAME, PASSWORD, PHONE, METAMASK_ADDRESS) VALUES (?, ?, ?, ?)",
                       (username, password, phone, metamask_address))
        conn.commit()
        conn.close()
        return redirect(url_for('user_login'))
